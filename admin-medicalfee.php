			...
			<blockquote class="layui-elem-quote">
				<?php
					...
					if($result&&$result->num_rows>0)
						...
						$sql="select form_no from FORMULATION where p_no=$patientID";
						$result=$mysqli->query($sql);
						if($result&&$result->num_rows>0)
							{
							...
							while($row = $result -> fetch_assoc())
								{
								$form_no=$row['form_no'];
								echo "<tr><td>$patientID</td><td>$patientName</td><td>$form_no</td><td><a href=\"admin-medicalfeebyid.php?formID=$form_no\" class=\"layui-btn layui-btn-normal layui-btn-sm\">查看费用</a></td></tr>";
								}
							echo "</tbody></table>";
							}
						else
							echo "<h2>该患者不存在处方单，请重新查询！</h2>";
						}
					else
