			...
			<blockquote class="layui-elem-quote">
                <?php
					...
					if($result&&$result->num_rows>0)
						...
						echo "<h2>患者 $patientName ，请查看您的病历！</h2>";
						echo "</blockquote><blockquote class=\"layui-elem-quote\">";
						$sql="select d_name,form_content from FORMULATION a,DOCTOR b where form_no=$formID and p_no=$patientID and a.d_no=b.d_no";
						//echo $sql;
						$result=$mysqli->query($sql);
						if($result&&$result->num_rows>0)
							{
							...
							echo "<label class=\"layui-form-label\">主治医生</label>
								<div class=\"layui-input-block\">
									<input type=\"text\" autocomplete=\"off\" class=\"layui-input\" readonly value=\"$d_name\" />
								</div>";
							echo "</div>";
							echo "<div class=\"layui-form-item\" pane>";
							...
							echo "<h2>不存在该病历或您无权查看该病历，请重新查看！</h2>";
						}
					else
						echo "<script>sessionStorage.isLogin=0;</script>";
				?>
			<script>document.write("<a href=\"patient-bingli.php?userID="+sessionStorage.userID+"\" class=\"layui-btn\">返回</a>");</script>
            </blockquote>
        </div>
    </div>
</div>
