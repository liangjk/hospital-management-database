			...
			<blockquote class="layui-elem-quote">
                <?php
					...
					if($result&&$result->num_rows>0)
						...
						echo "<h2>患者 $patientName ，请查询您的病房费用！</h2>";
						...
						if($result&&$result->num_rows>0)
							...
							echo "<table lay-filter=\"resultTable\">
									<thead>
										<tr>
											<th lay-data=\"{field:'roomID',sort:true,totalRowText: '费用合计：'}\">病房ID</th>
							...
							while($row = $result -> fetch_assoc())
								...
								echo "<tr><td>$ward_no</td><td>$hos_indate</td><td>$hos_outdate</td><td>$ward_fee</td></tr>";
								}
							echo "</tbody></table>";
							}
						else
				...
				?>
			<script>document.write("<a href=\"patient-roomfee.php?userID="+sessionStorage.userID+"\" class=\"layui-btn\">返回</a>");</script>
            </blockquote>
        </div>
    </div>
</div>
