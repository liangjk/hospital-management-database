			...
			<blockquote class="layui-elem-quote">
                <?php
					...
					if($result&&$result->num_rows>0)
						...
						echo "<h2>患者 $patientName ，请查询您的挂号信息！</h2>";
						echo "</blockquote><blockquote class=\"layui-elem-quote\">";
						$registers=array();
						date_default_timezone_set('Asia/Shanghai');
						for($date=strtotime($beginTime);$date<=strtotime($endTime);$date+=86400)
							{
							$sql="select reg_no,reg_date,d_name,dep_name from REGISTER a,DOCTOR b,DEPARTMENT c where p_no=$patientID and UNIX_TIMESTAMP(reg_date)=$date and a.d_no=b.d_no and b.dep_no=c.dep_no";
							$result=$mysqli->query($sql);
							if($result)
						...
						if(count($registers))
							...
							echo "<table lay-filter=\"resultTable\">
								<thead>
									<tr>
										...
										<th lay-data=\"{field:'doctorName'}\">医生姓名</th>
										<th lay-data=\"{field:'doctorDep',sort:true}\">医生科室</th>
							...
							foreach($registers as $row)
								...
								$d_name=$row['d_name'];
								$d_dep=$row['dep_name'];
								echo "<tr><td>$reg_no</td><td>$reg_date</td><td>$d_name</td><td>$d_dep</td><td><a href=\"patient-bingliquery?patientID=$patientID&regID=$reg_no\" class=\"layui-btn layui-btn-normal layui-btn-sm\">查看病历</a></td></tr>";
								}
							echo "</tbody></table>";
							}								
						else
							echo "<h2>该时间段内没有您的挂号信息，请重新输入！</h2>";
						}
					else
						echo "<script>sessionStorage.isLogin=0;</script>";
				?>
			<script>document.write("<a href=\"patient-guahao.php?userID="+sessionStorage.userID+"\" class=\"layui-btn\">返回</a>");</script>
            </blockquote>
        </div>
    </div>
</div>
